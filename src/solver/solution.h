 ...
 */
PROTOTYPE_FOR_UNIT_TEST(struct bounds* bounds_compute(const struct matrix* A));

/**
 * Free the bounds previously allocated by 'bounds_get'.
 */
PROTOTYPE_FOR_UNIT_TEST(void bounds_destroy(struct bounds* bounds));

/**
 * Set the list the empty cells.
 */
PROTOTYPE_FOR_UNIT_TEST(void solution_set_empty(const struct configuration* cfg,
 ...
 *  + bounds.
 */
PROTOTYPE_FOR_UNIT_TEST(void solution_set_mines(const struct configuration* cfg,
                                                const struct matrix* A,
                                                const struct bounds* bounds,
                                                struct solution* solution));
